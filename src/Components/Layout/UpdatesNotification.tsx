...
export default function UpdatesNotification() {
    ...
    useEffect(() => {
        ...
        async function fetchUpdates() {
            try {
                ...
                const q = query(collectionRef, where("userRef", "==", auth.currentUser?.uid))
                ...
                if (docSnap) {
                    docSnap.forEach(item => {
                        ...
                        updates.forEach((manga: { id: string, name: string, imgUrl: string }) =>
                            temp.push({
                                id: manga.id,
                                name: manga.name,
                                img: manga.imgUrl
    ...
    return (isLoading ? <Spinner /> :
        <div className="fixed top-28 left-5 w-full max-w-sm px-4">
                                            ...
                                            data.map((item) => (
                                                ...
                                                >
                                                    <div className="flex h-10 w-10 shrink-0 items-center justify-center text-white sm:h-12 sm:w-12">
                                                        <img src={item.img} alt="manga banner" className='max-w-[34px]' />
                                                    </div>
                                                    <div className="ml-4">
